...
doLoupe = function(e,imgEl,addStyle,toolBar,idList) {
	...
	if (t > jQ(window).height()-loupeHeight-63) { 
		t = jQ(window).height()-loupeHeight-63;
	}
...
loadDlDetails = function(dlType,dlTarget,dateTarget,id) {
	...
	jQ.ajax(url)
		.done(function(data) {
			...
			var dateTargetEl=jQ("#"+dateTarget);
			dateTargetEl.text(lastSale);
			...
			if (result) {
				var dlTable=jQ("table.reporttable:first",html);
				if (dlTable.length==0) {
					dlTable=jQ("table:eq(1)",html);
				}
				if (dlTable.length>0) {
					var title="Most recent " + (dlType?dlType:"Regular DLs");
					jQ("tr:first",dlTable).html("<th colspan='2'>"+title+"</th>");
					var t = dateTargetEl.position().top-jQ(window).scrollTop();
					var container=jQ("<div class='theasis_adpSalesTable' id='theasis_adpSalesTable_"+dlType+"'/>").html(dlTable.html()).css({display:"none",position:"absolute",left:100,top:100,"z-index":"15000"}).appendTo("body");
					dateTargetEl.mouseover(function(e){
						var x = e.pageX-20;
						var y = e.pageY-20;//-jQ(window).scrollTop();
						if (jQ(window).width()-x < 400) {
							x -= 400;
							if (x<0) {
								x=0;
							}
						}
						container.mouseout(function(){container.css({display:"none"})});
						container.css({display:"block",left:x,top:y});
					});	
				}
			}
		})
